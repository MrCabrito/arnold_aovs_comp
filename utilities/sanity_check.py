...
def AOV_check(layers_dict: dict) -> dict|None:
    ...
    wrong_data = dict()
    # Start progress bar
    task = create_progress_task('Searching for correct passes in the read nodes selected')
    ...
    for read_node in read_nodes:
        ...
        if missing_layers:
            wrong_data[read_node['name'].value()] = missing_layers
    ...
    if wrong_data:
        ...
        error_message = 'There are some missing passes\n'
        for read_node, missing_layers in wrong_data.items():
            error_message += '{0} -> {1}\n'.format(read_node, ', '.join(missing_layers))
